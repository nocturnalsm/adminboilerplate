...
export default function UserForm({data, open, roles, onClose, onError, onSuccess}) {
      ...
      const submit = async () => {
          try {
            const response = await axiosInstance({
                ...
                url: `/users${formData.id !== '' ? '/' + formData.id : ''}`,
                data: {
                    name: formData.name,
    ...
    return (
        <Dialog open={open} onClose={handleClose}>
          <FormDialogTitle onClose={handleClose}>Users</FormDialogTitle>
          {formData ? (
            <DialogContent>
              <TextField
                autoFocus
              ...
              <Autocomplete
                ...
                onChange={(event, selected) => {
                  handleChange({roles: selected})
                }}
                options={roles}
                renderOption={(props, option) => (
                  <li {...props} key={`options_roles_${option.id}`}>
                      {option.name}
                  </li>
                  )
                }
