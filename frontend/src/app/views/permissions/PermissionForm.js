...
export default function PermissionForm({data, open, onClose, onError, onSuccess}) {
      ...
      const submit = async () => {
          try {
            const response = await axiosInstance({
                ...
                url: `/permissions${formData.id !== '' ? '/' + formData.id : ''}`,
                data: {
                    name: formData.name
                }
            });
    ...
    return (
        <Dialog open={open} onClose={handleClose}>
          <FormDialogTitle onClose={handleClose}>Permissions</FormDialogTitle>
          {formData ? (
            <DialogContent>
              <TextField
                autoFocus
